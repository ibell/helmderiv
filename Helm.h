...
public:
    ...
    GenHelmDeriv(GenHelmDerivCoeffs &&coeffs) : coeffs(coeffs){ init(); };
    GenHelmDeriv(const GenHelmDerivCoeffs &coeffs) : coeffs(coeffs){ init(); };
    ...
    void calc(const double tau, const double delta, GenHelmDerivDerivs &derivs, const std::size_t Ntau_max = 3, const std::size_t Ndelta_max = 3)
    {
        ...
        if (use_delta_ld) {
            if (all_ld_integers){
                double _delta = delta;
                delta_to_ld = coeffs.ld_as_int.unaryExpr( [_delta](int _i){return powi(_delta, _i);} );
            }
            else{
